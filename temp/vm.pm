...
#loop eterno do programa
	for (my $opcode = "nothing"; $opcode ne "END"; $pcounter++) {
		...
		#verifica compatibilidade do opcode e valor
		$opcode = $1 if ($programa[$pcounter][0] =~ /([a-zA-Z]{2,4})/);
		$valor = $1 if ($programa[$pcounter][1] =~ /([0-9]+)/);

        #mostrando conteudo das variaveis
		# $valor? print "$opcode $valor\n": print "$opcode\n";
